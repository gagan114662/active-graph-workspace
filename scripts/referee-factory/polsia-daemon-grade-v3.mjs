...
for (const id of [1, 2, 3]) {
  const sol = `/tmp/polsia-d3-${id}/daemon.py`;
  const lp = path.join(ROOT, "frames", "referee", `polsia-daemon-v3-builder-${id}-${ts}.proof.jsonl`);
  ...
  L.note("provenance", "operator", `LLM-AGENT BUILD: blind builder-${id} (workflow w31gmr484 (API-skeleton round)). Acceptance referee-authored.`);
