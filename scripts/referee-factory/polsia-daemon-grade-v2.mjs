...
for (const id of [1, 2, 3]) {
  const sol = `/tmp/polsia-d2-${id}/daemon.py`;
  const lp = path.join(ROOT, "frames", "referee", `polsia-daemon-v2-builder-${id}-${ts}.proof.jsonl`);
  ...
  L.note("provenance", "operator", `LLM-AGENT BUILD: blind builder-${id} (workflow wrwpwzfbg (contract-clarified re-dispatch)). Acceptance referee-authored.`);
