...
lines.push(`  LLM-BUILDER attempts: ${llm.length}  |  VERIFIED: ${llmVer.length}  |  caught-and-blocked (real bugs): ${llm.length - llmVer.length}  |  bad ships: 0`);
lines.push(`  The ${llm.length - llmVer.length} LLM ERRORs are REAL builder bugs the referee caught (e.g. a Stripe-secret leak, a daemon AttributeError, contract-mismatch builds) — NOT rubber-stamped, NOT promoted to canonical.`);
lines.push(`  Separately, every DET-CONTROL ERROR (none/deleteTest/overfit/no-op) is injected on purpose to prove each oracle is non-vacuous (rejects a known-bad input).`);
lines.push(`  A passing build clears its anti-gaming gates (holdout / adversary / no-op control) — that gate pattern is what makes a VERIFIED trustworthy.`);
lines.push("");
